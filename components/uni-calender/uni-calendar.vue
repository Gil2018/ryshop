	...
	<view>
		<view class="uni-calendar__box">
			<view class="uni-calendar__wrapper">
				...
				<view class="sigin-desc uni-flex">
					<!-- 签到说明 -->
					<view class="sigin-desc-left uni-flex">
						<view class="uni-flex">
							<text class="markicon"></text>
							<text>已签到</text>
						</view>
						<view class="uni-flex">
							<text class="markicon disabledmark"></text>
							<text>未签到</text>
						</view>
						<view class="uni-flex">
							<text class="markicon curmark"></text>
							<text>今日</text>
						</view>
					</view>
					<view class="sigin-desc-right">
						<text>签到规则</text>
					</view>
				</view>
			</view>
		...
		methods: {
			...
			dataBefor(id, types) {
				...
				if (this.slide === 'none') {
					...
					this.indoorbuy.api_common.getAPI('/membersign/signlog', {data:this.canlender.year + '-' + this.canlender.month},function(res){
					  if(res.status==200){
						  that.signForm=res.data;
						  for(var item in res.data.log){
							  if(res.data.log[item].date!==undefined){
								  that.selected.push({date:res.data.log[item].date})
							  }
						  }
					  }
					})
